...
function updateDisplay() {
  ...
  // Update top bar (always visible) stats
  document.getElementById('topGold').textContent = Math.floor(gameState.gold);
  ...
  if (sellBtn) {
    sellBtn.disabled = sellAmount <= 0 || sellAmount > gameState.food;
    sellBtn.textContent = `Sell ${sellAmount} Food → +${(sellAmount * BALANCE.FOOD_TO_GOLD_RATE).toFixed(1)} Gold`;
  }
}
